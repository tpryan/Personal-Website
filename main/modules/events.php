...
function generateEventHTML($lanyrd_content,$count){
	...
	for ($i=0; $i<$count; $i++){
		...
		if (array_key_exists($i,$lanyrd_content) && array_key_exists('end',$lanyrd_content[$i]) && strlen($lanyrd_content[$i]['end']) > 0 ){
		...
		$entry .= "//storage.googleapis.com/terrenceryan.com/img/flags/" . strtolower($lanyrd_content[$i]['location_fixed']['country_code']) . ".svg";
		$entry .= '" />' . "\n";
		$entry .= '				</address>' . "\n";
...
function extracts_geo_details($googleinfo){
	...
	for ($i=0; $i<count($address_components); $i++){
		...
		if($address_components[$i]['types'][0] == "administrative_area_level_1"){
			if ( isset($results["city"]) && strlen($results["city"])  > 0){
				$results["state"] = $address_components[$i]['short_name'];
			} else {
				$results["city"] = $address_components[$i]['short_name'];
			}	
